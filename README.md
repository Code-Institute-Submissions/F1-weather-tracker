...
The goal for the website is to provide Formula 1 fans with a single place that contains all the relevant weather information for upcoming races in the 2021 Formula 1 calendar.
...
- As a **visitor**, I would like **to know when track sessions are supposed to start in my local time**, so that **I can prevent missing the session by accident**.
...
This website uses a single page layout, so everything is either visible already or at maximum 2 clicks away depending on your device. On smaller screen sizes you will be able to switch between 3 hour, 24 hour and 7 day weather forecasts by clicking a button. To see a visual sitemap [click here](wireframes/visual-sitemap.png).
...
To see separate image files of the wireframes check the wireframes/seperate-wireframe-images folder.
...
- Clicking on an event item changes the information displayed in the race countdown timer, track information, event time schedule, 3 hour weather forecast, 24 hour weather forecast and 7 day weather forecast features.
...
- Will be displayed below the race countdown timer and right of the event time schedule on large screen sizes. On smaller screen sizes the 3 hour weather forecast will be displayed below the weather forecast navigation.
...
- Will be displayed below the track information and the event time schedule and right of the race calendar on large screen sizes. On smaller screen sizes the 24 hour weather forecast will be displayed below the weather forecast navigation.
...
- Will be displayed below the 3 hour weather forecast and right of the 24 hour weather forecast on large screen sizes. On smaller screen sizes the 7 day weather forecast will be displayed below the weather forecast navigation.
- Textual weather forecast information such as the minimal temperature, maximum temperature, weather description and precipitation for the next 7 days at the selected race event location are displayed on a per day basis.

**Weather forecast navigation**:

- Only visible at smaller screen sizes, will be displayed below the race countdown timer.
- There are at least 3 buttons, clicking on them changes which type of weather forecast feature is displayed below.
- Additional buttons might be implemented here, such as a button to switch between Celsius or Fahrenheit.
...
- Percentage based chance of rain data could be added to both the 24 hour and the 7 day weather forecast sections.
