...
On basically all the Apple devices that I've tested on [Browserstack](https://www.browserstack.com/) the calendar event is [not displayed](testing/testing-img/ipad-air4-safari14-portrait.png), even when using Safari 14, same as in the old version of Samsung Internet. While using the same device in landscape mode [the calendar is actually visible](testing/testing-img/ipad-air4-safari14-landscape.png) and functions normally. On older iOS devices the website is completely broken, check out the [iPhone S6](testing/testing-img/iphone6S-safari9.png) for example. None of the Apple devices that I've tested showed the calendar event when in portait mode, [even when using Google Chrome](testing/testing-img/iphone12pro-chrome.png) instead of Safari. The images also don't work unless you are using Safari 14, this was no surprise since https://caniuse.com/webp already showed me that the WebP image format is poorly supported on Safari. I confirmed this by checking both [Safari 13](testing/testing-img/ipad-safari13.png) and [Safari 14](testing/testing-img/ipad-safari14.png) on the same device. I have no clue though why the calendar event is not displayed in portait mode, but this isn't necessarily an iOS only issue since the same issue happened on the old Samsung Internet version on an Android device too.

**Opera Version 74.0.3911.160 (64-bit):**

Everything is working and displaying as intended. [(screenshot)](testing/testing-img/opera-test.png)

**Samsung internet Version 9.2.00.70:**

The race calendar event is not displayed which means there is no nagivation to switch between race events. This issue was also seen at more older browser versions tested on https://www.browserstack.com/ and this same issue was present on most iOS devices that I've tested. [(screenshot)](testing/testing-img/sams-int-v9.jpg)

**Samsung internet Version 13.2.2.4:**

Everything is working and displaying as intended. [(screenshot 1)](testing/testing-img/sams-int1-v13.jpg) [(screenshot 2)](testing/testing-img/sams-int2-v13.jpg)

**Brave Browser Version 1.18.75:**

Everything is working and displaying as intended. [(screenshot)](testing/testing-img/brave-test1.jpg)
...
To test the website for mobile responsiveness I've been using the [Google Chrome devtools](testing-screenshots/chrome-devtools.png) throughout the coding of the media queries. I manually used the sliders to go through different screen sizes. I've also used all of the [pre-configured screen sizes](testing-screenshots/devtools-phones.png) in the devtools to test if things looked okay. I've also used https://responsivedesignchecker.com/ for some additional mobile responsiveness testing throughout the building of the website, since the Google Chrome devtools can behave a bit strange at times it's good to test responsiveness with more than a single tool.
...
![HTML validator warnings](testing/testing-img/label-warning-HTML.png)

These warnings were easily fixed by adding role="navigation" to the same div elements, after which the code [passed without warning or errors](testing/testing-img/HTML-pass.png).

### W3C - CSS Validation Service

I've put my style.css file through the [W3C CSS validator](https://jigsaw.w3.org/css-validator/) and it [passed without errors](testing/testing-img/CSS-pass.png) on the first try.
...
![JavaScript validator warnings](testing/testing-img/JS-validator.png)
...
- Images don't display when using Safari 13 or older versions, this is because only Safari 14 supports the WebP image format. ([see screenshot](testing/testing-img/ipad-safari13.png)).

- The calendar event (which is necessary for navigation between events) doesn't display on (nearly) all iOS devices with smaller screen sizes. So iOS users can only see data of the closest race event. ([see screenshot](testing/testing-img/iphone12pro-chrome.png)).

- On older iPhones the website seems to be completely broken. ([see screenshot](testing/testing-img/iphone6S-safari9.png)).
